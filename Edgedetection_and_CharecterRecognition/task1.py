...
def parse_args():
    ...
    parser.add_argument(
        "--img_path", type=str, default="D:/SEM2/CVIP/Proj1/project1/data/proj1-task1.jpg",
...
def convolve2d(img, kernel):
    ...
    """
    k_flip = flip2d(kernel)
    img_padded = zero_pad(img,1,1)
    img_conv = []
    m=0
    n=0
    sum=0
...
def edge_magnitude(edge_x, edge_y):
    ...
    for i in range(0,len(edge_x)):
        for j in range(0,len(edge_x)):
            edge_mag[i][j] = math.sqrt((edge_x[i][j]*edge_x[i][j])+(edge_y[i][j]*edge_y[i][j]));
            if edge_mag[i][j] < min:
                min = edge_mag[i][j]
            if edge_mag[i][j] > max:
                max = edge_mag[i][j]
    ...
    for i in range(0,len(edge_x)):
        for j in range(0,len(edge_x)):
            edge_mag[i][j]= (edge_mag[i][j]-diff)/diff;
            if edge_mag[i][j] <0:
                edge_mag[i][j] = 0;
    #raise NotImplementedError
    return edge_mag
